  ...
  <div>
    <div style="margin-right: 1.5%;background-color: rgb(255, 255, 255);width: 95%;height: 650px;border-radius: 8px;box-shadow: 0px 5px 13px rgb(218, 218, 218);display: flex;">
        <div style="width: 80%;height: 100%;">
            <iframe  style="background-color: black;width: 100%;height: 100%;border: none;" :src=vplayer.list[0].video>

            </iframe >
        </div>
        <div style="width: 20%;height: 30%;background-color: rgb(237, 237, 240);">
            <div style="margin-top: 5%;border-left: 2px solid red;">
                <h3 style="color: rgb(66, 66, 66);margin-left: 10%;" >{{ form.movName }}</h3>
            </div>
            <div>
            ...
            <div style="background-color: rgb(255, 255, 255);width: 100%;height: 220%;">
                <div style="margin-top:10%;width: 70%;margin-left: 15%;height: 35px;">
                    <div style="margin-top: 25%;">
                        <button style="margin-top: 5%;border: 1px solid rgb(236, 236, 236);width: 100%;height: 35px;color: black;border-radius: 8px;margin-left: 1%;background-color: white;"   v-for="(video, index) in forms" :key="index" @click="pageclick(video.id,video.pnum)">第 {{ video.pnum }} 集</button>
                    </div>
                </div>
            </div>
        </div>
...
<script>
  ...
  import porRequest from "@/utils/porRequest";
    ...
    created(){
            ...
            const pnums = this.$route.query.pnum
            request.get("/mov/movlistId/" + id).then(res =>{
                this.form = res.data
            })
            porRequest.get("/por/portfolioV/" + pnums + "/" + id).then(res =>{
